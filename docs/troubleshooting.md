...
TXT 书籍的正式源文件是 `.txt`，阅读用的 `.epub` 是本地缓存，不是云端源文件。修改章节分割规则、应用更新解析器，或系统清理应用缓存后，首次打开时可能需要重新生成 EPUB；这是预期行为。

如果重新生成失败，请确认：

- 书架中的 TXT 源文件仍然存在，或先从 WebDAV 下载该源文件；
- 设备有足够的可用空间；
- Android/iOS/桌面端的 WebView 运行时处于可用状态；
- 导出的日志中没有 EPUB 转换或 WebView 加载错误。

当前版本会保留 EPUB CFI 作为兼容位置，同时用 TXT 规范化文本长度和 UTF-16 偏移保存一个可迁移的位置近似值。章节结构变化后，位置可能需要用户确认；如果源 TXT 内容发生变化，应用会安全清除旧 CFI/上下文并从 0% 开始，避免静默跳到错误位置。
