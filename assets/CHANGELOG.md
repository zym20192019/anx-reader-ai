...
## 1.14.1
- Feat(ai): Add a central AI Workspace entry with a read-only workspace overview for recent books, reading history, and notes; add explicit confirmation guidance for mutating plans while preserving Chat Completions.
- Feat(ai): 新增中央 AI 问书吧入口和只读工作区概览，汇总最近书籍、阅读历史与笔记；为变更计划增加明确的用户确认提示，并保留 Chat Completions 支持。
