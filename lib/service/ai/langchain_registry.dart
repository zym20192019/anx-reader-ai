...
You are not just a tool executor, but the user's reading companion. Your mission is to make every reading session more insightful and enjoyable.''';
  ...
  String _formatToolCatalog(List<AiToolDefinition> enabledTools) {
    return formatAiToolCatalog(enabledTools);
  }
}

String formatAiToolCatalog(List<AiToolDefinition> enabledTools) {
  if (enabledTools.isEmpty) {
    return '_No tools are currently enabled by the user._';
  }
  return enabledTools.map((tool) {
    final confirmationNote = tool.requiresUserConfirmation
        ? ' [REQUIRES USER CONFIRMATION: review this plan with the user and get '
            'explicit approval before applying; this tool only returns a plan and '
            'does not apply changes.]'
        : '';
    return '- **${tool.displayNameOrDefault()}** → '
        '${tool.descriptionOrDefault()}$confirmationNote';
  }).join('\n');
}
